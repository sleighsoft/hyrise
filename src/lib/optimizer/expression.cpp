...
void Expression::reorder_columns(const std::vector<ColumnID> & column_id_mapping) {
  if (_column_id) {
    _column_id = column_id_mapping[*_column_id];
  }

  for (auto& expression : _expression_list) {
    expression->reorder_columns(column_id_mapping);
  }

  if (_left_child) {
    _left_child->reorder_columns(column_id_mapping);
  }
  if (_right_child) {
    _right_child->reorder_columns(column_id_mapping);
  }
}
