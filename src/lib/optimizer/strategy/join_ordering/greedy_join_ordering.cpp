...
std::shared_ptr<AbstractASTNode> GreedyJoinOrdering::run() {
  ...
  for (size_t edge_idx = 0; edge_idx < _input_graph->edges().size(); ++edge_idx) {
    ...
    _edges_by_vertex_id[edge.vertex_ids.first].emplace_back(edge_idx);
    _edges_by_vertex_id[edge.vertex_ids.second].emplace_back(edge_idx);
  }
  ...
  auto cheapest_vertex_costs = _input_graph->vertices()[0].node->get_statistics()->row_count();

  for (auto vertex_idx = JoinVertexID{1}; vertex_idx < _input_graph->vertices().size(); ++vertex_idx) {
    auto costs = _input_graph->vertices()[vertex_idx].node->get_statistics()->row_count();
   ...
   */
  _current_root = _input_graph->vertices()[initial_vertex_idx].node;
  _emit_vertex_predicates(initial_vertex_idx);
  ...
  for (size_t join_plan_size = 1; join_plan_size < input_vertices.size(); ++join_plan_size) {
    ...
    for (auto edge_idx : neighbourhood_edge_indices) {
      auto join_cost = _cost_join(_current_root, edge_idx);
...
//    const auto left_subtree_column_count = _current_root->output_column_count();
     ...
     */
    _left_column_id_of_vertex[join_vertex_ids.second] = make_column_id(_current_root->output_column_count());
    ...
    new_root->set_left_child(_current_root);
    ...
    for (const auto& edge_idx : predicate_edge_indices) {
      ...
      const auto left_column_id =
          _left_column_id_of_vertex[predicate_edge.vertex_ids.first] + predicate_edge.column_ids->first;
      const auto right_column_id =
          _left_column_id_of_vertex[predicate_edge.vertex_ids.second] + predicate_edge.column_ids->second;
     ...
     */
    _emit_vertex_predicates(join_vertex_ids.second);
  }
...
std::vector<size_t> GreedyJoinOrdering::_update_neighbourhood(std::set<size_t>& neighbourhood_edges,
  ...
  for (const auto& edge_idx : neighbourhood_edges) {
    ...
    if (edge.vertex_ids.first == vertex_ids.second || edge.vertex_ids.second == vertex_ids.second) {
...
float GreedyJoinOrdering::_cost_join(const std::shared_ptr<AbstractASTNode>& left_node, size_t edge_idx) const {
  ...
  if (edge.join_mode == JoinMode::Inner) {
    ...
    const auto join_stats = left_node->get_statistics()->generate_predicated_join_statistics(
        new_vertex.node->get_statistics(), JoinMode::Inner, join_column_ids, *edge.scan_type);
    return join_stats->row_count();
  } else {
    DebugAssert(edge.join_mode == JoinMode::Cross, "Bug. Was expecting a cross join here.");

    const auto join_stats = left_node->get_statistics()->generate_cross_join_statistics(new_vertex.node->get_statistics());
    return join_stats->row_count();
  }
}
...
std::pair<ColumnID, ColumnID> GreedyJoinOrdering::_get_edge_column_ids(size_t edge_idx,
  ...
  if (edge.vertex_ids.second == right_vertex_id) {
    return std::make_pair(make_column_id(_left_column_id_of_vertex[edge.vertex_ids.first] + edge.column_ids->first),
                          edge.column_ids->second);
  }
  return std::make_pair(make_column_id(_left_column_id_of_vertex[edge.vertex_ids.second] + edge.column_ids->second),
...
std::set<size_t> GreedyJoinOrdering::_extract_vertex_neighbourhood(JoinVertexID vertex_idx) {
  ...
  for (const auto& edge_idx : _remaining_edge_indices) {
    ...
    if (edge.vertex_ids.first == vertex_idx || edge.vertex_ids.second == vertex_idx) {
...
std::pair<JoinVertexID, JoinVertexID> GreedyJoinOrdering::_order_edge_vertices(const JoinEdge& edge) const {
  auto new_vertex_idx = edge.vertex_ids.first;
  auto contained_vertex_idx = edge.vertex_ids.second;

  if (_left_column_id_of_vertex[edge.vertex_ids.first] == INVALID_COLUMN_ID) {
    Assert(_left_column_id_of_vertex[edge.vertex_ids.second] != INVALID_COLUMN_ID,
...
void GreedyJoinOrdering::_emit_vertex_predicates(JoinVertexID vertex_id) {
  const auto& vertex = _input_graph->vertices()[vertex_id];

  for (const auto& predicate : vertex.predicates) {
    auto new_root = std::make_shared<PredicateNode>(make_column_id(predicate.column_id +_left_column_id_of_vertex[vertex_id]), predicate.scan_type, predicate.value);
    new_root->set_left_child(_current_root);
    _current_root = new_root;
  }
}
