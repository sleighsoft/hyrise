...
bool ast_contains_join_edge(const std::shared_ptr<const AbstractASTNode>& node,
                               ...
                               const std::shared_ptr<const AbstractASTNode>& descendant_b,
  ...
  return ast_contains_join_edge(
    node,
    make_column_id(first_column_id_a + column_id_a),
    make_column_id(first_column_id_b + column_id_b),
    scan_type
  );
}
...
bool ast_contains_join_edge(const std::shared_ptr<const AbstractASTNode>& node,
  ...
  if (node->type() == ASTNodeType::Join) {
    ...
    const auto left_column_count = join_node->left_child()->output_col_count();
    ...
    if (join_node->join_mode() == JoinMode::Inner) {
      ...
      if (static_cast<size_t>(column_id_b) >= left_column_count) {
        const auto in_column_order = join_node->join_column_ids() == std::make_pair(column_id_a,
                                                                                    make_column_id(column_id_b -
                                                                                                   left_column_count)) &&
      ...
      if (static_cast<size_t>(column_id_a) >= left_column_count) {
        const auto in_flipped_column_order = join_node->join_column_ids() == std::make_pair(column_id_b,
                                                                                            make_column_id(column_id_a -
                                                                                                           left_column_count)) &&
    ...
    if (static_cast<size_t>(column_id_a) >= left_column_count &&
        static_cast<size_t>(column_id_b) >= left_column_count) {
      return ast_contains_join_edge(join_node->right_child(),
                                    make_column_id(column_id_a - left_column_count),
                                    make_column_id(column_id_b - left_column_count),
                                    scan_type);
    }
  }
...
ColumnID ast_get_first_column_id_of_descendant(const std::shared_ptr<const AbstractASTNode>& node,
  ...
  if (node->right_child()) {
    ...
    if (column_id != INVALID_COLUMN_ID) {
      return make_column_id(column_id + node->left_child()->output_col_count());
    }
  }
