...
class JoinOrderingBaseTest : public ::testing::Test {
 public:
  JoinOrderingBaseTest() {
     ...
     *
     */
    JoinGraph::Vertices vertices_abcde = {JoinVertex{_table_node_a},
                                          JoinVertex{_table_node_b},
                                          JoinVertex{_table_node_c},
                                          JoinVertex{_table_node_d},
                                          JoinVertex{_table_node_e}};
     ...
     */
    JoinGraph::Vertices vertices_abcde_cross = {JoinVertex{_table_node_a},
                                                         JoinVertex{_table_node_b},
                                                         JoinVertex{_table_node_c},
                                                         JoinVertex{_table_node_d},
                                                         JoinVertex{_table_node_e}};
     ...
     */
    JoinGraph::Vertices vertices_abcde_complete_cross = {JoinVertex{_table_node_a},
                                          JoinVertex{_table_node_b},
                                          JoinVertex{_table_node_c},
                                          JoinVertex{_table_node_d},
                                          JoinVertex{_table_node_e}};
 ...
 protected:
  ...
  std::shared_ptr<JoinGraph> _make_clique_join_graph(const std::vector<std::shared_ptr<AbstractASTNode>>& vertex_nodes) {
    JoinGraph::Edges edges;

    for (auto vertex_idx_a = JoinVertexID{0}; vertex_idx_a < vertex_nodes.size(); ++vertex_idx_a) {
      for (auto vertex_idx_b = make_join_vertex_id(vertex_idx_a + 1); vertex_idx_b < vertex_nodes.size(); ++vertex_idx_b) {
    ...
    for (const auto& vertex_node : vertex_nodes) {
      vertices.emplace_back(JoinVertex{vertex_node});
    }

    return std::make_shared<JoinGraph>(std::move(vertices), std::move(edges));
  }

  std::shared_ptr<JoinGraph> _make_chain_join_graph(const std::vector<std::shared_ptr<AbstractASTNode>>& vertex_nodes) {
    JoinGraph::Edges edges;

    for (auto vertex_idx_a = JoinVertexID{1}; vertex_idx_a < vertex_nodes.size(); ++vertex_idx_a) {
